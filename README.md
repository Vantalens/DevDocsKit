...
- 个人开发者路径与团队协作路径并重的双体系规范
- 阶段性代码审核、标签化 Issue 报告和发布签署机制
- 强制测试、检查和覆盖率要求，覆盖率门槛不低于 80%
- 模块化开发约束，避免 AI 过度超前和一次性堆叠复杂功能
- Web 全栈、桌面/本地工具、AI/Agent 工具的项目类型分流清单
- 小阶段 PR、Issue+本地阶段报告双轨记录和关键代码提交确认
...
- 主路径：个人开发路径或团队开发路径。
- 项目类型：Web 全栈、桌面/本地工具、AI/Agent 工具。
- 冻结策略：先冻结关键闭环，再按小阶段持续补充规格。
...
- 开发前阅读相关开发文档，并确认个人/团队路径和 Web/桌面/AI 项目类型
- 大范围修改前使用 codegraph 或等效工具梳理项目
- 按模块边界开发，禁止无边界堆叠
- 不得过度超前实现未来功能
- 每个开发阶段完成后必须执行代码审核，问题进入 issue 和本地阶段报告
...
- report review findings as labeled issues and local phase reports
...
- 修改执行入口、项目类型分流、AI 协作、PR、Issue、测试门禁或发布规则时，必须同步检查个人版、团队版、通用规范和 AI 入口是否冲突。
- 个人版和团队版可以保留场景差异，但不得与通用规范的质量红线、提交规则和安全红线冲突。
...
- v2.2.0: Add dual personal/team paths, project-type routing, staged freeze, small-stage PR, issue plus local report tracking, and AI executor rules.
