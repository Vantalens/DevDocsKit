...
它的目标不是增加文档负担，而是把需求、架构、模块拆分、开发阶段、代码审核、Issue 报告、测试覆盖率、发布门禁和复盘机制整理成一套可复用的项目工作台。

## 适合谁使用

- 使用 Codex、Claude Code、AI Coding Agent 或 codegraph 进行开发的个人开发者
...
It is built for projects that use tools such as Codex, Claude Code, codegraph, AI coding agents, and human review workflows.
